...
class Node:
    ...
    def SendFrame(self):
        ...
        # 判断为可以发送的条件  满足IFG 没有消息正在发 有消息要发  信道不拥堵
        if self.FreeTime > Node.IFG and self.sending == False and len(
                self.frame) != 0 and Node.Crowd == False:  # 发送中
            self.sending = True
